...
- **Confirmation:** `TransactionRepository.confirmPendingAndApplyBalance` uses a guarded D1 batch to apply an ordinary pending row once. Linked MCP transfer review pairs use `resolvePendingTransferPair` to confirm or decline both legs together and apply both balance deltas once; other linked transfers cannot use pending confirmation. Preserve repeat/concurrent-confirmation behavior and the prohibition on confirming future dates. Date decisions use the browser's local-calendar `X-Client-Date`, with UTC fallback in the API; do not replace this with an unconditional UTC date.
...
- The only writes are expiring proposal storage, pending income/expense and linked cash-transfer review draft creation, previewed edits or declines of unresolved MCP review drafts and transfer pairs, and append-only forecast snapshots. Do not add posting, confirmation, hard deletion, account-balance mutation, investment writes, or arbitrary SQL tools without an explicit feature request changing that boundary.
...
- **Transfer review workflow:** prepare validates 1–20 cash-to-cash transfers and previews both native-currency legs. Different currencies require an explicit positive received amount; never infer it from FX quotes. The effective rate is display-only and a separate fee cannot be inferred from the two amounts alone. Same-currency legs must be equal. Creation writes reciprocal pending `mcp_review` rows in one D1 batch with no balance change. The app and MCP review list show each pair as one item. The app edits, posts, or declines both legs atomically. MCP transfer correction tools preview and atomically edit or decline both unresolved legs after explicit confirmation; ordinary correction tools remain limited to unlinked drafts. Pending edits keep balances and forecasts unchanged, require unlocked cash accounts, and preserve exact source and destination amounts. Future dates stay pending until the client calendar permits confirmation.
