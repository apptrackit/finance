...
- Use a Node version compatible with the locked tooling: Node 20.19+ on the 20.x line, 22.13+ on the 22.x line, or 24+. CI currently selects Node 20; the README's Node 18 prerequisite is stale.
- Read the relevant entry point and tests before editing. Package scripts, current source, and the complete migration sequence take precedence over historical README examples. `api/schema.sql` and `api/ARCHITECTURE.md` referenced there do not exist.
