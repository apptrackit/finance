...
- The only writes are expiring proposal storage, pending income/expense and linked cash-transfer review draft creation, previewed edits or declines of unresolved unlinked MCP review drafts, and append-only forecast snapshots. Do not add posting, confirmation, hard deletion, account-balance mutation, investment writes, or arbitrary SQL tools without an explicit feature request changing that boundary.
