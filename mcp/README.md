...
To correct a mistake in an existing draft, call `list_mcp_review_drafts`, following every cursor before claiming the list is complete. It shows each ordinary draft or linked cash transfer pair once. For ordinary income/expense drafts, call `prepare_mcp_review_draft_corrections` with 1–20 edits or declines, then `apply_mcp_review_draft_corrections` after explicit confirmation of the complete before/after preview. For transfers, use `prepare_mcp_transfer_corrections` and `apply_mcp_transfer_corrections` the same way. Either transfer leg ID identifies the pair; source and destination accounts and both native amounts are shown in each preview. A transfer edit or decline always applies to both legs. Proposals expire after 24 hours; stale app or MCP edits require a fresh list and preview. A decline cancels the draft or both legs and removes the item from the active review queue. Corrections do not post transactions, change balances, or add upcoming projections.

For a cash-to-cash transfer, call `prepare_mcp_transfer_drafts` with 1–20 source/destination account IDs, a positive `amount` sent, dates, and optional notes. When currencies differ, also supply the positive `amount_to` received in the destination account's native currency; preparation rejects a missing destination amount. For same-currency accounts, `amount_to` can be omitted and must equal `amount` if supplied. The preview shows both named legs and currencies, their exact amounts, a display-only effective rate (destination units per source unit, rounded to 10 significant digits), and duplicate warnings. No rate service determines either amount. The amounts alone do not reveal a separate exchange fee; that would require an agreed reference rate or an explicit fee. After explicit confirmation of the whole preview, call `create_mcp_transfer_drafts` with the opaque proposal ID alone. The 24-hour proposal creates reciprocal pending MCP review rows atomically and idempotently. The app shows one review item per pair; you can edit its accounts, native amounts, date, and note while pending. Changing accounts requires both amounts to be entered. The app or MCP saves both legs together without changing balances. Confirmation in the app posts both sides and changes both balances once; declining in the app or MCP cancels both with no balance change. A future-dated pair cannot be confirmed before its date.
...
| `prepare_mcp_transfer_drafts` | Preview 1–20 cash transfer pairs, requiring an explicit received amount across currencies, and store a 24-hour proposal |
| `create_mcp_transfer_drafts` | After explicit confirmation, atomically create linked pending transfer review pairs |
| `list_mcp_review_drafts` | Cursor-paginated unresolved MCP review items, one per ordinary draft or transfer pair; excludes ordinary upcoming rows |
| `prepare_mcp_review_draft_corrections` | Preview 1–20 unlinked income/expense edits or declines, storing a 24-hour proposal |
| `apply_mcp_review_draft_corrections` | After explicit confirmation, atomically apply an ordinary draft proposal or reject stale targets |
| `prepare_mcp_transfer_corrections` | Preview 1–20 transfer-pair edits or declines, including both native amounts, and store a 24-hour proposal |
| `apply_mcp_transfer_corrections` | After explicit confirmation, atomically edit or decline both transfer legs or reject stale targets |
